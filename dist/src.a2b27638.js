  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      if (content.includes("Tutorial") && !contatinerExist) {
        var _newElement = document.createElement("section");
        container.appendChild(_newElement);
        contatinerExist = true;
      }
      if (content.includes("Tutorial")) {
        var newTutorial = document.createElement("article");
        newTutorial.innerText = content;
        newTutorial.classList.add(".tutorial");
        articles.removeChild(article);
        newElement.appendChild(newTutorial);
      }
    }
  } catch (err) {
    _iterator2.e(err);
